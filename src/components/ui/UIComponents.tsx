...
export function Modal({ isOpen, onClose, title, children, size = "default" }: ModalProps) {
  ...
  const maxW = { sm: "380px", default: "520px", lg: "680px", xl: "800px" }[
    size
  ];
  return (
    <div
      ...
      {options.map((opt) => (
        <button
          ...
          style={{
            ...
            background:
              value === opt.value ? "var(--primary)" : "var(--bg-elevated)",
            color: value === opt.value ? "#fff" : "var(--text-secondary)",
            boxShadow:
              value === opt.value ? "var(--shadow-glow-primary)" : "none",
          }}
        >
          {opt.label}
        </button>
...
export function TagList({ tags = [], max = 3 }: { tags?: string[]; max?: number }) {
  ...
  return (
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
      {shown.map((t) => (
        <span key={t} className="badge badge-primary" style={{ fontSize: 11 }}>
          #{t}
        </span>
      ))}
    </div>
...
export function RatingStars({ rating, size = 14 }: { rating: number; size?: number }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
      {[1, 2, 3, 4, 5].map((s) => (
        <span
          ...
          style={{
            fontSize: size,
            color:
              s <= Math.round(rating) ? "var(--gold)" : "var(--text-muted)",
          }}
        >
          ★
        </span>
      ...
      <span
        style={{ fontSize: 12, color: "var(--text-secondary)", marginLeft: 3 }}
      >
        {rating}
      </span>
    </div>

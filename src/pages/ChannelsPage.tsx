...
import { formatNumber as formatCount } from "../utils/formatter";
import {
  SectionHeader,
  FilterChips,
  RatingStars,
...
const TIER_CONFIG = {
  ...
  gold: {
    label: "Gold",
    color: "var(--gold)",
    gradient: "linear-gradient(135deg,var(--gold),#fb923c)",
  },
  platinum: {
    label: "Platinum",
    color: "var(--accent)",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      >
        ...
        <div style={{ position: "relative" }}>
          ...
          >
            Paid channels give you direct access to curated insights, exclusive
            live sessions, and private communities.
          </p>
        </div>
      </div>
      ...
      {subscribedChannels.size > 0 && (
        ...
        >
          ...
          >
            {subscribedChannels.size} active subscription
            {subscribedChannels.size !== 1 ? "s" : ""}
          </span>
        </div>
      )}
...
function ChannelCard({ channel, delay }: ChannelCardProps) {
  const { subscribedChannels } = useSocial();
  const { openModal } = useUI();
  const isSubbed = subscribedChannels.has(channel.id);
  const tier = TIER_CONFIG[channel.tier as TierKey] || TIER_CONFIG.silver;
  ...
  return (
    ...
    >
      ...
      >
        <StatChip
          icon={<UsersIcon style={{ width: 12, height: 12 }} />}
          text={`${formatCount(channel.subscribers)} subscribers`}
        />
        <StatChip
          icon={<DocumentTextIcon style={{ width: 12, height: 12 }} />}
          text={`${channel.posts} posts`}
        />
        <span
          style={{
            background: tier.gradient,
      ...
      >
        ...
        {channel.perks.length > 3 && (
          ...
          >
            +{channel.perks.length - 3} more perks
          </span>
        )}
      </div>
...
function StatChip({ icon, text }: { icon: ReactNode; text: ReactNode }) {
  return (
    <span
      style={{
        display: "flex",

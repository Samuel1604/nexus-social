...
import { SectionHeader, TagList, EmptyState } from '../components/ui/UIComponents';
...
export default function GroupsPage() {
  const { groups, joinedGroupIds, toggleJoinGroup } = useSocial();
  const { openModal } = useUI();
  const [tab, setTab] = useState('all');

  const shown = tab === 'mine'
    ? groups.filter(g => joinedGroupIds.includes(g.id))
    : groups;
      ...
      <div className="tab-bar">
        {TABS.map(t => (
          <button key={t.value} className={`tab-item ${tab === t.value ? 'active' : ''}`} onClick={() => setTab(t.value)}>
            {t.label}
            {t.value === 'mine' && joinedGroupIds.length > 0 && (
              <span className="badge badge-primary" style={{ marginLeft: 8 }}>{joinedGroupIds.length}</span>
            )}
          </button>
        ))}
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {shown.map((group, i) => (
            <GroupCard
              ...
              onJoin={() => toggleJoinGroup(group.id)}
              onLeave={() => toggleJoinGroup(group.id)}
              delay={i * 60}
            />
          ))}
        </div>
...
function GroupCard({ group, joined, onJoin, onLeave, delay }: GroupCardProps) {
  return (

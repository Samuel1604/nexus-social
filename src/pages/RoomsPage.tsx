...
export default function RoomsPage() {
  const { rooms, joinedRoomIds, toggleJoinRoom } = useSocial();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {liveRooms.map((room, i) => (
              <RoomCard key={room.id} room={room} joined={joinedRoomIds.includes(room.id)} onJoin={() => toggleJoinRoom(room.id)} onLeave={() => toggleJoinRoom(room.id)} delay={i * 60} />
            ))}
          </div>
        </section>
      )}
      ...
      {upcomingRooms.length > 0 && (
        <section>
          <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: 'var(--text-secondary)', marginBottom: 14, textTransform: 'uppercase', letterSpacing: '0.05em', fontSize: 12 }}>
            Scheduled
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
...
function RoomCard({ room, joined, onJoin, onLeave, delay }: RoomCardProps) {
...
function gradientColors(cls: string) {

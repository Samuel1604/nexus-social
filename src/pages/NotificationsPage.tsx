...
export default function NotificationsPage() {
  const { notifications, unreadCount, markAllNotificationsRead, markNotificationRead } = useSocial();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <SectionHeader
        ...
        action={
          unreadCount > 0 && (
            <button className="btn btn-ghost btn-sm" onClick={markAllNotificationsRead} style={{ gap: 5, color: 'var(--primary-light)' }}>
...
function NotifItem({ notif, onRead, isLast }: NotifItemProps) {
  const cfg = TYPE_CONFIG[notif.type as NotificationType] || TYPE_CONFIG.system;
  const { Icon } = cfg;
